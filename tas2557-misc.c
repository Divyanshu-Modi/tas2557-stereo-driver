	...
	switch(pTAS2557->mnDBGCmd)
	{
		...
		case TIAUDIO_CMD_BITRATE:
		{
			...
			if(count == 1){
				...
				tas2557_get_bit_rate(pTAS2557, 
					pTAS2557->mnCurrentChannel, &bitRate);
		...
		case TIAUDIO_CMD_DACVOLUME:
		{
			...
			if(count == 1){
				...
				ret = tas2557_get_DAC_gain(pTAS2557, 
					pTAS2557->mnCurrentChannel, &volume);
				if(ret >=0){						
		...
		case TIAUDIO_CMD_BITRATE:
		{
			...
			if(count == 3){
				...
				tas2557_set_bit_rate(pTAS2557, 
					pTAS2557->mnCurrentChannel, p_kBuf[2]);
			}
		}
		break;
